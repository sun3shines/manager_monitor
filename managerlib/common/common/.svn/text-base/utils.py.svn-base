...
import managerlib.common.common.exceptions
from managerlib.common.common.http import is_success, is_redirection, HTTP_NOT_FOUND, \
...
def lock_path(directory, timeout=10, timeout_class=None):
    ...
    if timeout_class is None:
        timeout_class = managerlib.common.exceptions.LockTimeout
    mkdirs(directory)
...
def lock_file(filename, timeout=10, append=False, unlink=True):
    ...
    while True:
        ...
        try:
            with managerlib.common.exceptions.LockTimeout(timeout, filename):
                while True:
                    try:
                        fcntl.flock(fd, fcntl.LOCK_EX | fcntl.LOCK_NB)
                        break
...
def iter_multipart_mime_documents(wsgi_input, boundary, read_chunk_size=4096):
    ...
    if got.strip() != boundary:
        raise managerlib.common.common.exceptions.MimeInvalid(
